...
function instagram_login_button_shortcode( $atts, $content ){
	...
	//add_action('receive_token', 'instagram_login_receive');
...
function instagram_login_receive(){ 
    wp_register_script( 'instagram_login_js', plugins_url('assets/js/app.js', __FILE__), array('jquery'));
	wp_enqueue_script( 'instagram_login_js' );
...
function ilogin_action(){
	...
	if($user){
		//if yes, logs in user with that username, no further auth is required (delegating that to Instagram's API)
...
add_shortcode( 'instagram-receive', 'instagram_login_receive' );
